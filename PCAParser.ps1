<#
	.SYNOPSIS
		A parser that converts the *.txt files in C:\Windows\appcompat\pca to CSV
		
		.DOCUMENTATION
		https://aboutdfir.com/new-windows-11-pro-22h2-evidence-of-execution-artifact/
	
	.DESCRIPTION
		A detailed description of the PCAParser.ps1 file.
	
	.PARAMETER inputPath
		Provide a path where recursively the artifacts that reside in C:\Windows\appcompat\pca exist
	
	.PARAMETER outputPath
		Provide a path where the CSV output will be placed for each parsed *.txt file in C:\Windows\appcompat\pca
	
	.NOTES
		===========================================================================
		Created with: 	SAPIEN Technologies, Inc., PowerShell Studio 2023 v5.8.223
		Created on:   	2023-06-17 23:03
...
(
	[Parameter(Mandatory = $true,
			   Position = 1,
			   HelpMessage = 'Provide a path where recursively the artifacts that reside in C:\Windows\appcompat\pca exist')]
	[string]$inputPath,
	[Parameter(Mandatory = $true,
			   Position = 2,
			   HelpMessage = 'Provide a path where the CSV output will be placed for each parsed *.txt file in C:\Windows\appcompat\pca')]
	[string]$outputPath
)
...
{
	Log -logFile $logFile -msg "Creating output directory $outputPath"
...
function Parse-PcaAppLaunchDic
{
	...
	Log -logFile $logFile -msg "Looking for $fileMaskPcaAppLaunchDic files"
	
	# Find all files named PcaAppLaunchDic.txt
	$files = Get-ChildItem -Path $inputPath -Filter $fileMaskPcaAppLaunchDic -Recurse -File
	
	# Check if $files is null
	if ($null -eq $files)
	{
		Log -logFile $logFile -msg "No $fileMaskPcaAppLaunchDic files found in $inputPath"
		return
	}
	
	$fileCount = $files.Count
	Log -logFile $logFile -msg "Found $fileCount $fileMaskPcaAppLaunchDic files to process"
	
	# Loop through each file and process it
	foreach ($file in $files)
	{
		Log -logFile $logFile -msg "Processing file $($file.FullName)"
		...
		Log -logFile $logFile -msg "$($file.Name) | File size: $fileSizeKB KB | Row Count: $rowCount"
		...
		{
			# Split the line into the path and timestamp
		...
		$outputFile = Join-Path -Path $outputPath -ChildPath "$($file.BaseName).csv"
		
		# Export the data to a CSV file
		Log -logFile $logFile -msg "Writing data to $outputFile"
		$data | Export-Csv -Path $outputFile -NoTypeInformation -Encoding ASCII
		
		# Retrieve file info
		$csvFileInfo = Get-Item -Path $outputFile
		
		# Compute file size in kilobytes and row count
		$fileSizeKB = [math]::Round($csvFileInfo.Length / 1KB, 2)
		$rowCount = $data.Count
		
		Log -logFile $logFile -msg "$($csvFileInfo.Name) has been successfully built! | File size: $fileSizeKB KB | Row Count: $rowCount"
	}
}
...
function Parse-PcaGeneralDb
{
	...
	Log -logFile $logFile -msg "Looking for $fileMaskPcaGeneralDb files"
	
	# Find all files named PcaGeneralDb*.txt
	$files = Get-ChildItem -Path $inputPath -Filter $fileMaskPcaGeneralDb -Recurse -File
	
	# Check if $files is null
	if ($null -eq $files)
	{
		Log -logFile $logFile -msg "No $fileMaskPcaGeneralDb files found in $inputPath"
		return
	}
	
	$fileCount = $files.Count
	Log -logFile $logFile -msg "Found $fileCount $fileMaskPcaGeneralDb files to process"
	...
	foreach ($file in $files)
	{
		...
		# Read the contents of the file with UTF8 encoding
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABC